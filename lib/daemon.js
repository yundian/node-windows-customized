...
var daemon = function(config){
  ...
  Object.defineProperties(this,{
    ...
    id: {
      enumerable: true,
      get: function(){
        return this.name;//this.name.replace(/[^\w]/gi,'').toLowerCase();
      }
    },
        ...
        fs.exists(dir,function(exists){
          ...
          fs.writeFile(path.resolve(dir,me.id+'.xml'),me._xml,function(){
            ...
            winsw.createExe(me.id,dir,function(){
              me.execute('"'+path.resolve(dir,me._exe + '.exe')+'" install',function(){
                sleep(2);
                me.emit('install');
              });
            });
    ...
    uninstall: {
      ...
      value: function(waitTime){
        ...
        var uninstaller = function(){
          ...
          me.execute('"'+path.resolve(me.root,me._exe + '.exe')+'" uninstall',function(error,stdout,stderr){
            if (error){
